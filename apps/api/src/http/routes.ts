...
export function registerRoutes(app: FastifyInstance, deps: CreateServerDeps): void {
  app.get("/v1/balances/:holder", async (request, reply) => {
    ...
    if (query.tokenAddress !== undefined) balanceQuery.tokenAddress = query.tokenAddress as `0x${string}`;
  ...
  app.get("/v1/transfers/:holder", async (request, reply) => {
    ...
    const transferQuery: TransferQuery = { holder: params.holder as `0x${string}`, limit: query.limit, offset: query.offset };
    if (query.chainId !== undefined) transferQuery.chainId = query.chainId;
    if (query.tokenAddress !== undefined) transferQuery.tokenAddress = query.tokenAddress as `0x${string}`;
    if (query.decryptionStatus !== undefined) transferQuery.decryptionStatus = query.decryptionStatus;
